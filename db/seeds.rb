...
@u.rushes << @r
@l = Location.create(:name => 'The Starting Line', :rush_id => @r.id, :description => "Welcome to the first ever City Rush! Blah blah blah.... As soon as you check in at this location, you'll see a clue. Race to that clue, and click Check in to log your location", :lat => 53.412128, :long => -113.49777, :rush_order => 1)
...
@l = Location.create(:name => 'Big Red', :rush_id => @r.id, :description => "You can find a little bit of Stockholm in Edmonton nearby this next checkpoint. The way here once had an ARC to it, winding between two man made lakes. Check in at the front fountain, where the circle's inside the square.", :lat => 53.443666, :long => -113.475876, :rush_order => 2)
@c = Clue.create(:description => "Head to the southern-most table you can find on the ring to get your first destination!", :location_id => @l.id, :lat => 53.410505, :long => -113.498822)

@l = Location.create(:name => 'Head West', :rush_id => @r.id, :description => "This peninsula is one of the most popular spots to spend some quality time with your best friend. Be as quick as a rabbit, and check in on the grassy island after you park.", :lat => 53.479871, :long => -113.607446, :rush_order => 3)
